...
class ASTBuilder(RAParserVisitor):
    ...
    def visitJoinExpr(self, ctx):
        ...
        op_type = ctx.getChild(1).symbol.type
        
        if op_type == RAParser.LEFT_OUTER_JOIN:
            return radb.ast.LeftJoin(left, cond, right)
        elif op_type == RAParser.RIGHT_OUTER_JOIN:
            return radb.ast.RightJoin(left, cond, right)
        elif op_type == RAParser.FULL_OUTER_JOIN:
            return radb.ast.FullJoin(left, cond, right)
        else:
            # Default to standard Inner/Natural Join for \join or ⋈
